...
export class ExampleMaker {
  ...
  private calculateQuotes(currentPosition: number): {
    ...
    const bidPrice = this.state.oraclePrice * (1 - bidOffsetBps / 10000);
    const askPrice = this.state.oraclePrice * (1 + askOffsetBps / 10000);
  ...
  private validateQuotes(quotes: {
    ...
    if (quotes.bidPrice <= 0 || quotes.askPrice <= 0) {
      log.warn("VALIDATION", "Invalid prices in quotes");
      return false;
    }

    if (quotes.bidPrice >= quotes.askPrice) {
      log.warn("VALIDATION", "Crossed quotes detected");
      return false;
    }
  ...
  private async updateOrders(quotes: {
    ...
    try {
      ...
      const bidOrderParams = {
        ...
        price: this.driftClient.convertToPricePrecision(quotes.bidPrice),
      ...
      const askOrderParams = {
        ...
        price: this.driftClient.convertToPricePrecision(quotes.askPrice),
        postOnly: PostOnlyParams.TRY_POST_ONLY,
        marketType: MarketType.PERP,
      };
      ...
      log.info("ORDERS", "Orders updated successfully", {
        bidPrice: quotes.bidPrice.toFixed(2),
        askPrice: quotes.askPrice.toFixed(2),
        spread:
          (
            ((quotes.askPrice - quotes.bidPrice) / this.state.oraclePrice) *
            10000
          ).toFixed(1) + "bps",
      });
    } catch (error) {
      log.error("ORDERS", "Failed to update orders", error as Error);
